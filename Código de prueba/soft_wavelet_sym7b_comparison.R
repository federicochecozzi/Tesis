...
wav1 <- fread("wav1b.csv", dec = ',',header = TRUE)

colnames(wav1) <- colnames(spectra)

wav2 <- fread("wav2b.csv", dec = ',',header = TRUE)
...
for (i in 1:100) {
  ...
  p <- df[c(0 + i,100 + i,200 + i),] %>% 
    ...
    mutate(Wavelength = as.numeric(Wavelength), 
           ...
           linetype = as.factor((id == "spectrum") * 1 + (id == "BayesShrink") * 2 + (id == "VisuShrink") * 3)) %>%
    ggplot() + 
